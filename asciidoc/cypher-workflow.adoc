...
The following example illustrates the concept of DBMS <<cypher-manual#query-transactions, Cypher Manual -> Transactions>> and shows how queries to multiple databases can be issued in one driver transaction. It is annotated with comments describing how each operation impacts database transactions.
