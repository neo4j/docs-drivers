...
For a *_single instance_*, this is limited to *_one user database_*, plus the `system` database.
...
For more information about managing multiple databases within the same DBMS,
refer to <<cypher-manual#neo4j-databases-graphs, Cypher Manual -> Neo4j databases and graphs>>
which has a full breakdown of the *Neo4j* data storage hierarchy.
