...
* `TRUST_SYSTEM_CA_SIGNED_CERTIFICATES` - Trust strategy for certificates that can be verified through the local system store.

* `TRUST_CUSTOM_CA_SIGNED_CERTIFICATES` - Only encrypted connections to Neo4j instances with certificates signed by a trusted certificate will be accepted.
To use this option you should configure the driver with a file containing the signed certificates one or more trusted X.509 certificates.

* `TRUST_ALL_CERTIFICATES` - Trust any certificate provided. This is intended for use with self-signed certificates. *We do not recommend using this setting for production environments.*
