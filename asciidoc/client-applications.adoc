...
=== Custom domain resolution and multiple initial routers

The 1.7 series drivers introduced the ability to intercept domain name resolution performed by a driver instance.
This can be useful to avoid or augment DNS and is carried out _before_ DNS itself.
...
The driver will automatically verify the server name and expiry date for any certificate received.
https://en.wikipedia.org/wiki/Server_Name_Indication[Server Name Indication (SNI)] is also enabled by default, allowing proxy middleware to route encrypted Bolt connections to the correct backend server.
