...
====
Additional configuration of the server is required when using a single instance of neo4j deployed on a remote machine
whilst using the `neo4j` scheme. To make the server aware on its deployment environment you need to configure
https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_dbms.default_advertised_addressAsciidoctor[`default_advertised_address`]
...
Each of the `neo4j` and `bolt` URIs permit variants that contain extra encryption and trust information.
