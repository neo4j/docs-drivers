...
The `HOST` and `PORT` values contain a logical hostname and port number targeting the entry point to the Neo4j service.
...
The optional `ROUTING_CONTEXT` allows for customization of the routing table and is discussed in more detail in <<driver-routing-context>>.

=== Schemes

The driver can be configured with 6 different URI schemes which cover a variety of
options. The two main groups are `neo4j` and `bolt`. The `neo4j` scheme targets deployments at a service level where the deployment that the driver is communicating with can be a single instance or a clustered and the driver will rout traffic accordingly.

The other group is the `bolt` URI scheme (e.g. `bolt://graph.example.com:7687`), which can be used when a single point-to-point connection is required.
This variant is useful for a subset of client applications (such as admin tooling) that need to be aware of individual servers, as opposed to those which require a highly available database service.

These groups can be combined with security setting in the following combinations:


|===
|URI Scheme |Description | Routing | Encryption | Certificate Checks

|`neo4j`
| Unencrypted connections to Neo4j as a service.
| Enabled
| Disabled
| N/A

|`neo4j+s`*
| Encrypted connections to Neo4j as a service with full certificate checks.
| Enabled
| Enabled
| Full Checks
...
|`neo4j+ssc`*
|Encrypted connections to Neo4j as a service with self-signed certificate checks.
| Enabled
| Enabled
| No Checks
...
|`bolt`
| Unencrypted connections to a single Neo4j instance.
| Disabled
| Disabled
| N/A
...
|`bolt+s`*
| Encrypted connection to a single Neo4j instance with full certificate checks.
| Disabled
| Enabled
| Full Checks
...
|`bolt+ssc`*
|Encrypted connection to a single Neo4j instance with self-signed certificate checks.
| Disabled
| Enabled
| No Checks
...
\* are only available since `v4.0.1` of the Java and .NET drivers and `4.0.2` of the JavaScript driver. For more details about security please see the <<driver-security,secuirty>> section.

[NOTE]
Schemes `neo4j` and `bolt` are configurable for encryption and trust through the programmatic <<driver-configuration, configuration API>>.
Other schemes cannot be used in conjunction with the configuration API.
...
Specify whether to use an encrypted connection between the driver and server. This cannot be configured in conjunction with: `neo4j+s`, `bolt+s` or `neo4j+ssc` `bolt+ssc` connection schemes.
Default: None
...
This cannot be configured in conjunction with: `neo4j+s`, `bolt+s` or `neo4j+ssc` `bolt+ssc` connection schemes.
+
