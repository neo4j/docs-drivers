...
*_Destroying a Driver Object will immediately shut down any connections previously opened via that Driver Object_*, by closing the associated connection pool.
...
*_The configuration details are immutable for the lifetime of the Driver Object._*
...
A connection URI *_identifies a graph database and how to connect to it_*.
...
====
If you are using a *_single instance_* of *Neo4j*, deployed on a remote machine whilst using the `neo4j` URI scheme,
*_you will need to complete additional configuration of the server_*.
