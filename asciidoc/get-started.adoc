...
| Python             | CPython 3.5 and above. Note that Python 2 support has been deprecated and was discontinued in the 4.x series drivers.
|===

[NOTE]
--
The 4.0 release of the Go driver is a work in progress. There is currently no release date available.

For more information on the 1.7 Driver documentation, including the 1.7 and 1.8 Go Driver, see {docs-base-uri}/driver-manual/1.7.
--
...
For information, see For information, see <<migration-guide#upgrade-driver, 4.0 Migration Guide -> Chapter 6. Upgrade Neo4j drivers>>.
...
The .Net Driver, by default, uses asynchronous methods.
If you want to use blocking calls, then there is a set of extensions that are contained in the `Neo4j.Driver.Simple` NuGet package.
...
[source, csharp]
----
include::{dotnet-examples}/ExamplesAsync.cs[tags=hello-world]
----
