...
It will be visible in the output of the procedures `<<operations-manual#query-management-list-queries, dbms.listQueries>>` and `<<operations-manual#query-management-list-transactions, dbms.listTransactions>>`. 
...
Each such exception is associated with a <<status-codes#status-codes, status code>> that describes the nature of the error and a message that provides more detail.
