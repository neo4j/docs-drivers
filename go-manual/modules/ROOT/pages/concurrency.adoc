...
<1> The channel `records_c` is where the query result records will be streamed to.
`queryToChannel()` writes to it, and the various `consumer`s read from it.
It is buffered so that the driver does not retrieve records faster than what the consumers can handle.
<2> A Goroutine runs the query to the Neo4j server with a xref:transactions.adoc[managed transaction].
...
<4> The channel `log` is where the consumers will comunicate on.
<5> A `sync.WaitGroup` is needed to know when all consumers will be done, and thus the `log` channel can be closed.
