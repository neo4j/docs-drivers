...
- if you receive a temporal object using one of  link:https://neo4j.com/docs/cypher-manual/5/functions/temporal/[_Cypher temporal functions_], you will get back the corresponding _driver type_ as displayed in the table below. You may then use `.Time()` on them to convert them into Go `time.Date` objects.
+
[options="header"]
|===
| Cypher Type      | Go Type
...
Cypher supports link:{neo4j-docs-base-uri}/cypher-manual/5/values-and-types/spatial/[spatial values] (points), and Neo4j can store these point values as properties on nodes and relationships.
