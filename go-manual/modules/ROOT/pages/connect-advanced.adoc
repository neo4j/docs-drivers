...
To work with rotating tokens, you need to provide a link:https://pkg.go.dev/github.com/neo4j/neo4j-go-driver/v6/neo4j/auth#TokenManager[`TokenManager`] instance when instantiating the `Driver`, rather than a static authentication token.
...
By using `TelemetryDisabled = false` upon connection, the driver will never send any telemetry data.
...
driver, err := neo4j.NewDriver(
    ...
    func(conf *config.Config) {
        conf.TelemetryDisabled = false
    })
defer driver.Close(ctx)
----

For more information on what information is transmitted, see link:https://pkg.go.dev/github.com/neo4j/neo4j-go-driver/v6/neo4j/config#Config.TelemetryDisabled[API documentation -> `Config.TelemetryDisabled`].
