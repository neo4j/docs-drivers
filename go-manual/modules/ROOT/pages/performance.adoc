...
Besides protecting from Cypher injections, this allows to leverage the database's query cache.
...
----
session := driver.NewSession(ctx, neo4j.SessionConfig{DatabaseName: "neo4j"})
session.Run(ctx, "MATCH (p:Person|Animal {name: $name}) RETURN p", nil)
----
...
----
session := driver.NewSession(ctx, neo4j.SessionConfig{DatabaseName: "neo4j"})
session.Run(ctx, "MATCH (p {name: $name}) RETURN p", nil)
----
...
session.ExecuteRead(ctx,
    func(tx neo4j.ManagedTransaction) (any, error) {
        result, err := tx.Run(ctx, "MATCH (p:Person) RETURN p", nil)
    })
----
...
[source, go]
----
session.ExecuteWrite(ctx,  // don't ask to write on a read-only operation
    func(tx neo4j.ManagedTransaction) (any, error) {
        result, err := tx.Run(ctx, "MATCH (p:Person) RETURN p", nil)
    })
----
