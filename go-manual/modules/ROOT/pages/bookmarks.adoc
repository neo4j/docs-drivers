...
func create_some_friends(driver neo4j.DriverWithContext):
    ...
    combine  // <2>
    with driver.session(bookmarks=saved_bookmarks) as session_c:  // <3>
...
<1> Collect the bookmarks from some sessions using the method `SessionWithContext.LastBookmarks()`
<2> Combine bookmarks from different sessions with `neo4j.CombineBookmarks()`
<3> Use them to initialize another session with the `Bookmarks` config parameter
