...
To link:https://neo4j.com/developer/kb/protecting-against-cypher-injection/[protect against Cypher injections], you should enclose the dynamic values in backticks and escape them yourself.Notice that Cypher processes Unicode, so take care of the Unicode literal `\u0060` as well.
