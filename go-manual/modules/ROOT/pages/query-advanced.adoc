...
Contrary to xref:query-simple.adoc[`.ExecuteQuery()`] and xref:transactions#managed-transactions[managed transactions], implicit transactions offer limited guarantees of automatic retrial.
As of version 6.1, implicit transactions are retried once on errors marked as idempotent by the server.
You can disable auto-retry on implicit transactions via the configuration option `DisableAutoCommitRetries` both at the link:https://pkg.go.dev/github.com/neo4j/neo4j-go-driver/v6/neo4j/config#Config.DisableAutoCommitRetries[driver] and link:https://pkg.go.dev/github.com/neo4j/neo4j-go-driver/v6/neo4j#SessionConfig.DisableAutoCommitRetries[session] level.
...
To ensure an implicit transaction is committed, consume all records (either by calling `.Consume(ctx)` on its result or by iterating over all records).
