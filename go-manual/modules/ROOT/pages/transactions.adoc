...
You can exert further control on transactions providing configuration callbacks to `.ExecuteRead()`, `.ExecuteWrite()`, and `.BeginTransaction()`.
Use them to specify:
...
It is thus recommended to call `driver.Close()` with the `defer` keyword as soon as you create a new session, to be sure it will be closed in all cases.
