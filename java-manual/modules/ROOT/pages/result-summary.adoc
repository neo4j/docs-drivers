...
The plan is then available as a link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/summary/Plan.html[`Plan`] object through the method `ResultSummary.queryPlan()` (or `ResultSummary.plan()` for versions < 6.2), and contains the list of link:https://neo4j.com/docs/cypher-manual/current/planning-and-tuning/operators/[Cypher operators] that would be used to retrieve the result set.
Use this information to locate bottlenecks or potential room for xref:performance.adoc[performance improvements] (for example through the creation of indexes).
...
result.summary()
        ...
        .ifPresentOrElse(
                queryPlan ->
                        System.out.println(queryPlan.arguments().get("string-representation")),
                () -> System.out.println("No query plan available"));
...
The plan is available as a link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/summary/QueryProfile.html[`QueryProfile`] object through the method `ResultSummary.queryProfile()` (or a link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/summary/ProfiledPlan.html[`ProfiledPlan`] object via `ResultSummary.profile()` for versions < 6.2).
...
result.summary()
        ...
        .ifPresentOrElse(
                queryProfile ->
                        System.out.println(queryProfile.arguments().get("string-representation")),
                () -> System.out.println("No query profile available"));
...
The method `ResultSummary.gqlStatusObjects()` returns an ordered set of GQL-compliant status objects.
