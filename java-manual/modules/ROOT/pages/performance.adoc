...
[source, java]
----
// send one single queries with all values inside
...
[source, java]
----
// send a lot of single queries, one for each value
...
- using the method `.withRouting(RoutingControl.READ)` in xref:query-simple.adoc#_request_routing[`Driver.execute_query()`] calls
...
driver.executableQuery("MATCH (p:Person) RETURN p")
    .withConfig(QueryConfig.builder()
        .withDatabase("neo4j")
        .withRouting(RoutingControl.READ)  // import org.neo4j.driver.RoutingControl  // mark-line
        .build())
    .execute();
----
...
Creating a safe execution context around a number of queries obviously yields an overhead that is not present if the driver just shoots queries at the server and hopes they get through.
