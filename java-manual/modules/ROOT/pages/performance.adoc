...
== Switch to Netty native transports
...
You need to provide a valid native dependency in your package manager.
The Netty version the driver depends on is `4.2`, and the Netty version you specify for the transport dependency must be compatible with that.

.Example dependency for native transports with Maven
[source, xml]
----
<dependency>
...
TCP Fast Open (TFO) enables the server to send data already during the TCP handshake (either during the Bolt or TLS handshake, depending on the encryption settings).
You enable TCP Fast Open via the driver configuration `withTryTcpFastOpen(true)`.
...
try (var driver = GraphDatabase.driver(dbUri, AuthTokens.basic(dbUser, dbPassword),
    Config.builder().withTryTcpFastOpen(true).build())) {
    driver.verifyConnectivity();
}
----

Both the client and the server must be configured for TFO to work. +
For more information, see link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/Config.ConfigBuilder.html#withTryTcpFastOpen(boolean)[API documentation -> `withTryTcpFastOpen`].
////
