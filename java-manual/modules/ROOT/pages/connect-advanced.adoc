...
Use link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/AuthTokens.html#custom(java.lang.String,java.lang.String,java.lang.String,java.lang.String,java.util.Map)[`AuthTokens.custom()`] to log into a server having a custom authentication scheme.
...
You need to provide a link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/AuthTokenManager.html[`AuthTokenManager`] instance when instantiating the `Driver`, rather than a static authentication token.
...
Use link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/ClientCertificateManagers.html[`ClientCertificateManagers.rotating()`] for both static and rotating certificates. +
The method takes a link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/ClientCertificates.html[`ClientCertificate`] instance. +
...
To change the driver's logging behavior, use the link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/Config.ConfigBuilder.html#withLogging(org.neo4j.driver.Logging)[`.withLogging()`] method when creating a `Driver` object.
...
If link:https://en.wikipedia.org/wiki/OCSP_stapling[OCSP stapling] is link:https://neo4j.com/docs/operations-manual/current/configuration/configuration-settings/#config_server.bolt.ocsp_stapling_enabled[enabled on the server], the driver can be configured to check for certificate revocations during SSL handshakes.
...
- link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/Config.TrustStrategy.html#withStrictRevocationChecks()[`.withVerifyIfPresentRevocationChecks()`] -- validate a certificate's stapling if available, but don't fail verification if no stapling is found.
- link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/Config.TrustStrategy.html#withStrictRevocationChecks()[`.withStrictRevocationChecks()`] -- validate a certificate's stapling, and fail verification if no stapling is found.
...
The driver can send anonymous usage statistics to the server it connects to, if the server requests those.
Telemetry is managed on the server-side via the configuration setting link:https://neo4j.com/docs/operations-manual/current/configuration/configuration-settings/#config_server.bolt.telemetry.enabled[`server.bolt.telemetry.enabled`].
By default, telemetry is disabled on self-managed instances and enabled on Aura instances.

By using `.withTelemetryDisabled(true)` upon connection, the driver will never send any telemetry data.

.Disable telemetry on the driver side
[source, java]
----
// import org.neo4j.driver.Config;
// import org.neo4j.driver.GraphDatabase;
...
For more information on what information is transmitted, see link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/Config.ConfigBuilder.html#withTelemetryDisabled(boolean)[API documentation -> `.withTelemetryDisabled()`].


== Further connection parameters

You can find all `Driver` configuration parameters in the link:https://neo4j.com/docs/api/java-driver/{java-driver-version}/org.neo4j.driver/org/neo4j/driver/GraphDatabase.html[API documentation -> driver.GraphDatabase].
