...
[NOTE]
The bearer authentication scheme requires link:{neo4j-docs-base-uri}/operations-manual/5/authentication-authorization/sso-integration[configuring Single Sign-On on the server].
...
If link:https://en.wikipedia.org/wiki/OCSP_stapling[OCSP stapling] is link:https://neo4j.com/docs/operations-manual/5/configuration/configuration-settings/#config_server.bolt.ocsp_stapling_enabled[enabled on the server], the driver can be configured to check for certificate revocations during SSL handshakes.
