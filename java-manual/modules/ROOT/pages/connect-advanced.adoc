...
The new `bolt+unix` URI scheme allows connecting to a Neo4j server >= 2025.08 over Unix sockets.
Use it to connect to an instance even when all standard Bolt threads are busy.

This feature requires a Neo4j server >= 2025.08 and to be link:https://neo4j.com/docs/operations-manual/current/configuration/configuration-settings/#config_server.bolt.unix_socket_enabled[enabled and configured] on the server first (including the socket's path).
...
To work with rotating tokens, you need to provide a link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/AuthTokenManager.html[`AuthTokenManager`] instance when instantiating the `Driver`, rather than a static authentication token.
...
Logging is supported via Java's link:https://docs.oracle.com/en/java/javase/21/core/java-logging-overview.html[`System.Logger`].
By default, you can control the driver's logging setup by tweaking the content of the file `$JAVA_HOME/conf/logging.properties`.
...
For log messages to appear in the console, you also need to adjust the console logging level.
...
[WARNING]
Log entries of level `FINER` can contain sensitive information (such as database credentials), so consider such logs confidential.
...
For more information on what information is transmitted, see link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/Config.ConfigBuilder.html#withTelemetryDisabled(boolean)[API documentation -> `.withTelemetryDisabled()`].
