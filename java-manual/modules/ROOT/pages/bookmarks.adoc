...
To ensure causal consistency among transactions executed partly with `.executableQuery()` and partly with sessions, retrieve the default bookmark manager for `ExecutableQuery` instances through `driver.executableQueryBookmarkManager()` and pass it to new sessions through the `.withBookmarkManager()` config method.
