...
| `VECTOR`      | link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/internal/value/VectorValue.html[`VectorValue`]
| `UUID`        | link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/internal/value/UUIDValue.html[`UUIDValue`]
|===
...
The type link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/internal/value/VectorValue.html[`VectorValue`] maps to the Cypher type link:https://neo4j.com/docs/cypher-manual/25/values-and-types/vector/[`VECTOR`].
Vector objects are stored as contiguous blocks of memory, containing homogeneous values.

You can create a vector from lists of `byte`, `short`, `int`, `long`, `float`, `double` via one of the link:++https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/Values.html#vector(byte[])++[`Values.vector()`] methods.
...
The Cypher type link:https://neo4j.com/docs/cypher-manual/current/values-and-types/uuid/[`UUID`] maps to link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/internal/value/UUIDValue.html[`UUIDValue`], basing on `java.util.UUID`.
...
var result = driver.executableQuery("CREATE (v:Value {id: $uid}) RETURN v.id AS uid")
    .withParameters(Map.of("uid", UUID.randomUUID()))
    .execute();

var uuidValue = result.records().get(0).get("uid");
...
The type link:https://neo4j.com/docs/api/java-driver/current/org.neo4j.driver/org/neo4j/driver/internal/value/UnsupportedTypeValue.html[`UnsupportedTypeValue`] is used for data types returned by Cypher queries, but that the driver doesn't recognize.
