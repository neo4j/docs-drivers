...
Since a connection error is a blocker for any subsequent task, the most common choice is to let the program crash should an exception occur during connection.
...
To connect to such an instance, you may either use the URI and authentication credentials explicitly in your application, or load the content of the connection file in the environment with `Dotenv.load()` and populate your local variables via `dotenv.get()`.
