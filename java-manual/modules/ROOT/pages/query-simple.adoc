...
Once you have xref:connect.adoc[connected to the database], you can run queries using <<Cypher>> and the method `Driver.executableQuery()`.
...
records.forEach(r -> {
    System.out.println(r);  // or r.get("name")
});

// Summary information
var summary = result.summary();  // <2>
System.out.printf("The query %s returned %d records in %d ms.\n",
        ...
        try (var driver = GraphDatabase.driver(URI, AuthTokens.basic(USER, PASSWORD))) {
            ...
            try {
                ...
                people.forEach(person -> {
                    var result = driver.executableQuery("MEGE (p:Person {name: $person.name, age: $person.age})")
                        .withConfig(QueryConfig.builder().withDatabase("neo4j").build())
