...
The type link:https://neo4j.com/docs/api/dotnet-driver/current/api/Neo4j.Driver.Vector.html[`Vector`] maps to the Cypher type link:https://neo4j.com/docs/cypher-manual/25/values-and-types/vector/[`VECTOR`].
...
The Cypher type `UUID` maps directly to link:https://learn.microsoft.com/en-us/dotnet/api/system.guid[`System.Guid`].
It can be used as a property value, query parameter, and return value in the same way as any other scalar type such as `INTEGER` or `BOOLEAN`.

[IMPORTANT]
`UUID` support requires a server that negotiates Bolt 6.1.

.Store and retrieve a UUID property
[source, csharp]
----
var original = Guid.NewGuid();
var result = await driver.ExecutableQuery(@"
    MERGE (n:Item {nodeId: $input})
    RETURN n.nodeId AS nodeId
    ")
    .WithParameters(new { input = original })
    .WithConfig(new QueryConfig(database: "<database-name>"))
    .ExecuteAsync();

var record = result.Result[0];
Console.WriteLine(record["nodeId"].As<Guid>());    // direct access
Console.WriteLine(record.AsObject<Item>().nodeId); // via object mapping
