...
[IMPORTANT]
Storing and retrieving `UUID` values requires Bolt 6.1.

.Store and retrieve a UUID property
[source, csharp]
----
var guid = Guid.NewGuid();
var ids = await driver.ExecutableQuery(@"
    MERGE (n:Item {id: $id})
    RETURN n.id AS id
    ")
    .WithParameters(new { id = guid })
    .WithConfig(new QueryConfig(database: "<database-name>"))
    .ExecuteAsync()
    .AsObjectsAsync((Guid id) => id);

Console.WriteLine(ids[0]);
Console.WriteLine(guid);   // same value
----
