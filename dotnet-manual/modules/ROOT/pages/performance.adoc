...
*Specify the target database on all queries*, both in xref:query-simple.adoc#database-selection[`Driver.ExecutableQuery()`] calls or xref:transactions.adoc#database-selection[when creating new sessions].
...
[TIP]
====
When using `.ExecutableQuery()`, you can process records as they arrive using the link:https://neo4j.com/docs/api/dotnet-driver/{dotnet-driver-version}/api/Neo4j.Driver.IExecutableQuery-2.WithStreamProcessor.html[`.WithStreamProcessor()`] method.
