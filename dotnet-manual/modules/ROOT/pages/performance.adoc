...
- using the config `routing: RoutingControl.Readers` in xref:query-simple.adoc#_request_routing[`IDriver.ExecutableQuery()`] calls
...
- using the config method xref:transactions.adoc#_request_routing[`.WithDefaultAccessMode(AccessMode.Read)` when creating a new session] (for explicit transactions).
...
for (int i=1; i<=10000; i++) {
    await driver.ExecutableQuery("CREATE (:Number {value: $value})")
