...
A query run may fail for a number of reasons, with different link:https://neo4j.com/docs/api/dotnet-driver/current/api/Neo4j.Driver.Neo4jException.html[exceptions] being raised.
Some of them are due to Cypher syntax errors, permission issues, or other forms of misconfiguration/misusage.

From version >= 5.28, the default exception format is GQL-compliant. For more information on the structure of the error object, see link:https://neo4j.com/docs/api/dotnet-driver/current/api/Neo4j.Driver.Preview.GqlErrors.IGqlErrorPreview.html[GqlErrorPreview].
