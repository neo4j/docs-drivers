...
var result = await driver
    .ExecutableQuery("CREATE (p:Person {name: $name})")  // <1>
...
<1> The Cypher query
<2> A map of xref:query-parameters[_query parameters_]
...
----
var result = await driver
    .ExecutableQuery("MATCH (p:Person {name: $name}) RETURN p")
    .WithConfig(new QueryConfig(database: "neo4j"))
    .WithParameters(new { name = "Alice" })
    .ExecuteAsync();
----
...
----
var result = await driver
    .ExecutableQuery("MATCH (p:Person) RETURN p.name")
    .WithConfig(new QueryConfig(database: "neo4j"))
    .ExecuteAsync();
----
