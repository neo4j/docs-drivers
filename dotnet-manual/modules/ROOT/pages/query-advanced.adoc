...
The most common use case for using `IAsyncSession.RunAsync()` is for importing large CSV files into the database with the link:{neo4j-docs-base-uri}/cypher-manual/5/clauses/load-csv[`LOAD CSV`] Cypher clause, and preventing timeout errors due to the size of the transaction.
...
For more information, see link:{neo4j-docs-base-uri}/cypher-manual/5/clauses/load-csv/[Cypher -> Clauses -> LOAD CSV].
