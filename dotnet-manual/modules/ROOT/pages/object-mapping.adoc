...
If a constructor parameter has a default value, the mapper uses it when the value is absent from the record.
If the value _is_ present but its value is incompatible with the parameter type, an exception is thrown.

[NOTE]
====
If `[MappingDefaultValue]` is applied to a parameter that also has a default value and the value is missing, the attribute's value takes precedence over the parameter default.
...
<.> `age` is absent from the record, so the constructor's default is used.
