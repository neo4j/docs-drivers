...
By default, xref:query-simple.adoc[`Driver.execute_query()`] returns an link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.EagerResult[`EagerResult`] object.
...
- `expand` -- If `True`, some data structures in the result will be recursively expanded and flattened. More info in the link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.Result.to_df[API documentation].
...
The graph transformer returns a link:https://neo4j.com/docs/api/python-driver/{python-driver-version}/api.html#neo4j.graph.Graph[`Graph`] object exposing the properties `nodes` and `relationships`, which are set views into xref:data-types#_node[`Node`] and xref:data-types#_relationship[`Relationship`] objects.
...
For more advanded scenarios, you can use the parameter `result_transformer_` to provide a custom function that further manipulates the link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.Result[`Result`] object resulting from your query.
