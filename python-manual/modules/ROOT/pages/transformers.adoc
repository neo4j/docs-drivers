...
The driver can transform the result into a Pandas DataFrame by setting `result_transformer_=neo4j.Result.to_df`.
This method requires the `pandas` library is installed.
...
The driver can transform the result into a collection of graph objects by setting `result_transformer_=neo4j.Result.graph`.
...
Inside a transformer function you can use xref:transactions.adoc#process-result[any of the `Result` methods].
