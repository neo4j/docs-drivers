...
It also contains a link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#resultsummary[`ResultSummary`] of the query result, with metadata and information about the result *(2)*, and the `keys` available in the returned rows *(3)*.
...
print(f"Query `{query}` returned {records_count} records in {time} ms.".format(
    query=summary.query, records_count=len(records),
    time=summary.result_available_after
))

print("Available keys are {keys}")  # ['name', 'age'] <3>
----
