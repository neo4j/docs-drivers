...
To interact with the database in an asyncronous way, create an `AsyncDriver` with `AsyncGraphDatabase()`. The workflow is very similar to the syncronous version, except that you must use `await` on all async calls, and define as `async` all functions that should be awaited.
