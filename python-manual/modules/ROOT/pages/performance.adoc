...
When submitting queries that may result in a lot of result records, don't retrieve all records at once.
...
- The client has to hold in memory all of 250 records.

|
- The server reads the first 100 records and sends them to the driver.
- The application can process records as soon as the first batch of 100 is transferred.
