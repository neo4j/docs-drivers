...
<2> With eager loading, the first record is available once the final result is consumed.
<3> Memory usage is larger with eager loading because the application will materialize a list of 250 records.
<4> The total running time is longer with eager loading because the thread is blocked until the final result is received and the results.
With lazy loading, the client may also stop requesting records after some condition is met (by calling `.consume()` on the `Result` object) to save time and resources.
====
