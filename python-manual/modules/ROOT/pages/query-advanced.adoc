...
As of version 6.2, implicit transactions are retried on errors marked as idempotent by the server (currently consisting of admission control errors only). You can disable auto-retry on implicit transactions via the configuration option link:https://neo4j.com/docs/api/python-driver/current/api.html#session-disable-auto-commit-retries-ref[`disable_auto_commit_retries`] both at the driver and session level.
