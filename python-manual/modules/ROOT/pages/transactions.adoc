...
When xref:query-simple.adoc[querying the database] with transaction functions, the driver automatically creates a *<<transaction>>*. You can easily include multiple queries in a single transaction function, but to design more involved use-cases it is important to understand how transactions work within the driver.
