...
The most common form is _managed transactions_, and you can think of them as a way of unwrapping the flow of `.execute_query()` and being able to specify its desired behavior in more places.
...
*Sessions are _not_ thread safe*: you can share the main `Driver` object across threads, but each thread should create its own sessions.
...
You create a managed transaction with the methods link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#neo4j.Session.execute_read[`Session.execute_read()`] and link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#neo4j.Session.execute_write[`Session.execute_write()`], depending on whether you want to retrieve data from the database or alter it.
Both methods take a _transaction function_ callback, which is responsible for carrying out the queries and processing the result.
...
Because the transaction might be re-run, *transaction functions must be _idempotent_* (i.e., they should produce the same effect when run several times), because you do not know upfront how many times they are going to be executed.
...
The decorator link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#neo4j.unit_of_work[`unit_of_work()`] gives further control on transaction functions.
...
If `key` is specified, only the given property is included; `default` allows to specify a value for nodes lacking that property.
...
- `strict==True` -- a link:https://neo4j.com/docs/api/python-driver/current/api.html#neo4j.exceptions.ResultNotSingleError[`ResultNotSingleError`] is raised.
