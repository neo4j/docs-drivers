...
By default, xref:query-simple[`Driver.execute_query()`] returns an link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#neo4j.EagerResult[`EagerResult`] object. It contains the result records as a list, so it is easy to loop through them.
...
def main():
    with GraphDatabase.driver(URI, auth=AUTH) as driver:
        ...
        visualize_result(result, nodes_text_properties)
