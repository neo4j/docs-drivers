...
You can run a query against the database using <<Cypher>> and the method `Session.execute_query()`.

[TIP]
`Driver.execute_query()` was introduced with the version 5.8 of the driver. +
For earlier versions, see xref:transactions.adoc[Run your own transactions].


== Write to the database

To create a node representing a person named `Alice`, use the `Session.execute_write()` method in combination with the Cypher clause link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/merge/[`MERGE`]:
...
where `records` contains the actual result as a list, `keys` contains the column names of the returned records (such as `name`), and `summary` is a link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#resultsummary[`ResultSummary`] object, containing a summary of execution from the server.


== Update the database

To update a node's information in the database, use the Cypher clauses `MATCH` and link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/set/[`SET`]:
...
Query parameters can be passed either as several keyword arguments, or grouped together in a dictionary as a single second argument:
...
Because `.execute_query()` can potentially raise a number of different exceptions, the best way to handle errors is to catch all exceptions in a single block:
