...
To update an entity's information in the database, use the Cypher clauses link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/match/[`MATCH`] and link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/set/[`SET`]:
...
An exception will be raised if the operation keeps failing after the configured link:https://neo4j.com/docs/api/python-driver/current/api.html#max-transaction-retry-time-ref[maximum retry time].
...
The actual _cause_ that triggered an exception is sometimes found in the optional GQL-status object `\\__cause__`, which is itself an exception (or `None`).
