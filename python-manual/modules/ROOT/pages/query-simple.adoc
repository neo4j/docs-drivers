...
where *(1)* specifies the Cypher query and *(2)* is a _query parameter_. These are wrapped in a transaction function *(3)*, which is passed a callback to `Session.execute_write()` together with an arbitrary numbers of keyword arguments *(4)*.
