...
When running queries with xref:query-simple.adoc[`Driver.execute_query()`], the execution summary is part of the default return value, as second object.
...
The plan is available under the property `ResultSummary.plan`, and contains the list of link:https://neo4j.com/docs/cypher-manual/current/planning-and-tuning/operators/[Cypher operators] that would be used to retrieve the result set.
Use this information to locate potential bottlenecks or potential performance improvements (for example the creation of indexes).
...
The property `ResultSummary.gql_status_objects` contains a sequence of link:{neo4j-docs-base-uri}/api/python-driver/current/api.html#gqlstatusobject[`GqlStatusObject`]s.
...
Use the parameters `notifications_min_severity` and/or `notifications_disabled_categories`/`notifications_disabled_classifications` to restrict the severity and/or category/classification of notifications that you are interested into.
...
From version 6.0, `notifications_disabled_categories` is deprecated.
