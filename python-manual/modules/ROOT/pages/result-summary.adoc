...
**Notice that once you ask for the execution summary, the transaction is closed and the result stream exhausted**.
...
If you are only interested in some of them, or if you want to disable them altogether, you can use the driver parameters `notifications_min_severity` and/or `notifications_disabled_categories` when creating a `Driver` instance in the first place (available from version 5.7).
