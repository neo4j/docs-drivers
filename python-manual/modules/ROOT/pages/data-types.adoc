...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#core-data-types[API documentation -- Core data types].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#temporal-data-types[API documentation -- Temporal data types].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/temporal.html#date[API documentation -- Temporal data types -- Date].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/temporal.html#time[API documentation -- Temporal data types -- Time].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/temporal.html#datetime[API documentation -- Temporal data types -- DateTime].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/temporal.html#duration[API documentation -- Temporal data types -- Duration].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/spatial.html[API documentation -- Spatial types].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/spatial.html#cartesianpoint[API documentation -- Spatial types -- CartesianPoint].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/types/spatial.html#wgs84point[API documentation -- Spatial types -- WSG84Point].


== Graph types

*Graph types are only passed as results and can't be used as parameters*.
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#graph-data-types[API documentation -- Graph types].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.graph.Node[API documentation -- Graph types -- Node].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.graph.Relationship[API documentation -- Graph types -- Relationship].
...
For full documentation, see link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.graph.Path[API documentation -- Graph types -- Path].
...
The type link:https://neo4j.com/docs/api/python-driver/current/types/vector.html#neo4j.vector.Vector[`Vector`] maps to the Cypher type link:https://neo4j.com/docs/cypher-manual/current/values-and-types/vector/[`VECTOR`].
`Vector` objects are stored as contiguous blocks of memory (`bytes`), containing homogeneous values encoded in big-endian order.

You can create a `Vector` from various data types.

.Some examples of valid `Vector` invocations
[source, python]
----
Vector([1, 2, 3], "i8")  # <.>
Vector(b"\x00\x01\x00\x02", "i16")  # <.>
Vector(b"\x01\x00\x02\x00", "i16", byteorder="little")  # <.>
Vector(numpy.array([1, 2, 3]))  # <.>
Vector(pyarrow.array([1, 2, 3]))  # <.>
----

<.> An iterable of floats/ints, and the type (link:https://neo4j.com/docs/api/python-driver/current/types/vector.html#neo4j.vector.VectorDType[`VectorDType`]) of the individual entries. +
Supported types are:
...
- `i32`: 32-bit integer
- `i64`: 64-bit integer
<.> Raw bytes, and the type (link:https://neo4j.com/docs/api/python-driver/current/types/vector.html#neo4j.vector.VectorDType[`VectorDType`]) to parse the bytes with.
<.> When providing raw bytes, `byteorder` is optional and defaults to `"big"`.
<.> A `numpy.ndarray`. No further parameters are accepted, and the `VectorDType` is inferred from the input.
<.> A `pyarrow.Array`. No further parameters are accepted, and the `VectorDType` is inferred from the input.
...
.Retrieve and work with a vector
[source, python]
----
from neo4j.vector import Vector
...
[IMPORTANT]
Because Python has no native `float32` type, vectors of `float32` converted into native Python lists become lists of `float64`, leading to potential representation imprecisions.
To retain the `float32` representation, transform the result vector into a numpy array (link:https://neo4j.com/docs/api/python-driver/current/types/vector.html#neo4j.vector.Vector.to_numpy[`Vector.to_numpy`]) or into a pyarrow array (link:https://neo4j.com/docs/api/python-driver/current/types/vector.html#neo4j.vector.Vector.to_pyarrow[`Vector.to_pyarrow`]).


[role=label--new-6.3 label--enterprise-edition]
[#uuid]
== UUID

The Cypher type link:https://neo4j.com/docs/cypher-manual/current/values-and-types/uuid/[`UUID`] maps directly to link:https://docs.python.org/3.10/library/uuid.html#uuid.UUID[`uuid.UUID`].

[NOTE]
The `UUID.is_safe` property is not preserved when a UUID is stored and retrieved from the database.

.Store and work with a UUID
[source, python]
----
import uuid

# Insert and read a UUID
uuid = driver.execute_query(
    "CREATE (d:Doc {id: $uuid}) RETURN d.id AS uuid LIMIT 1",
    uuid=uuid.uuid1(),
    database_="neo4j",
).records[0]["uuid"]
...
The type link:https://neo4j.com/docs/api/python-driver/current/types/other.html#neo4j.types.UnsupportedType[`UnsupportedType`] is used for data types returned by Cypher queries, but that the driver doesn't recognize.
