...
The driver type link:https://neo4j.com/docs/api/python-driver/current/types/vector.html#neo4j.vector.Vector[`Vector`] maps to the Cypher type link:https://neo4j.com/docs/cypher-manual/current/values-and-types/vector/[`VECTOR`].
...
driver.execute_query("CREATE (d:Doc {embedding: $vector})", vector=v)
----
...
The type link:[`UnsupportedType`] is used for data types that the driver doesn't know about, most likely because the client version is too old with respect to the server version.
