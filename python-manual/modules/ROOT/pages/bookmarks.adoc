...
This guarantees that the `Person` nodes `session_c` wants to act on actually exist.
...
[TIP]
The use of bookmarks can negatively impact performance, since the query is forced to wait for the latest changes to be propagated across the cluster.
If possible, try to group queries within a single transaction, or within a single session.


== Mix `.execute_query()` and sessions

To ensure causal consistency among transactions executed partly with `.execute_query()` and partly with sessions, create the session with `bookmark_manager=driver.execute_query_bookmark_manager`.
