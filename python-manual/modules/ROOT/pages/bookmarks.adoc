...
You can subclass the link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.api.BookmarkManager[`BookmarkManager`] interface to implement a custom bookmark manager, or use the default implementation provided by the driver through link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.GraphDatabase.bookmark_manager[`GraphDatabase.bookmark_manager()`]. When implementing a bookmark manager, keep in mind that all methods must be concurrency safe.

The details of the interface can be found in the link:{neo4j-docs-base-uri}/api/python-driver/{python-driver-version}/api.html#neo4j.api.BookmarkManager[API documentation].
