= Coordinate parallel transactions with bookmarks

When working with a Neo4j cluster, the driver automatically enforces <<causal_consistency>> for transactions within the same session, which guarantees that a query is able to read changes made by previous queries. However, if your application runs multiple xref:transactions.adoc[transactions] in parallel, you can use _bookmarks_ to have one transaction wait for the result of the other to be propagated across the cluster before running its own work.
