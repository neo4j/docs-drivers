= Coordinate different transactions with bookmarks

When working with a Neo4j cluster, <<causal_consistency>> is enforced by default in most cases, which guarantees that a query is able to read changes made by previous queries. However, if you xref:transactions.adoc[take control of transactions] or if your application is running concurrent transactions, you might need to use _bookmarks_ to avoid the cluster falling back to <<eventual_consistency>>.
