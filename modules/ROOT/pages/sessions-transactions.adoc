...
:description: This section describes how to create units of work and provide a logical context for that work. 
...
In languages where xref:terminology.adoc#term-thread-safety[thread safety] is an issue, a session should _not_ be considered thread-safe.
...
Parameter values should adhere to xref:cypher-values.adoc[Working with Cypher values].
...
Of these, only xref:sessions-transactions.adoc#driver-transactions-transaction-functions[transaction function] can be automatically replayed on failure.
...
Such a transaction consists of only one Cypher statement, cannot be automatically replayed on failure, and cannot take part in a xref:sessions-transactions.adoc#driver-transactions-causal-chaining[causal chain].
...
However, Auto-commit transactions are the only way to execute `link:docs/cypher-manual/3.5/query-tuning/using/index.adoc#query-using-periodic-commit-hint[USING PERIODIC COMMIT]` Cypher statements.
...
This retry capability can be xref:client-applications.adoc#driver-configuration-max-retry-time[configured] on Driver construction.
...
It will be visible in the output of the procedures `link:docs/operations-manual/3.5/monitoring/query-management/index.adoc#query-management-list-queries[dbms.listQueries]` and `link:docs/operations-manual/3.5/monitoring/transaction-management/index.adoc#transaction-management-list-transactions[dbms.listTransactions]`.
It will also get logged to the `link:docs/operations-manual/3.5/monitoring/logging/query-logging/index.adoc[query.log]`.
...
A specified timeout overrides the default timeout defined by the configuration setting `link:docs/operations-manual/3.5/reference/configuration-settings/index.adoc#config_dbms.transaction.timeout[config_dbms.transaction.timeout]`.

For usage patterns, please refer to the xref:get-started.adoc#driver-api-docs[API documentation] for your specific language.

[[driver-transactions-cypher-errors]]
=== Cypher errors

When executing Cypher, it is possible for an exception to be thrown by the Cypher engine.
Each such exception is associated with a link:docs/status-codes/3.5/index.adoc#status-codes[status code] that describes the nature of the error and a message that provides more detail.
...
Causal chaining is carried out by passing xref:terminology.adoc#term-bookmark[bookmarks] between transactions.
...
Internally, a bookmark is passed from server to client on successful completion of a transaction and back from client to server on start of a new transaction, even if the transaction is an xref:sessions-transactions.adoc#driver-transactions-auto-commit[auto-commit transaction].
...
[NOTE]
--
If you use the {driver-version} driver to connect to a Neo4j database that is version 3.4 or earlier, xref:sessions-transactions.adoc#driver-transactions-auto-commit[auto-commit transactions] cannot take part in the causal chain.
...
In the general case, access mode should always be specified at transaction level, using xref:sessions-transactions.adoc#driver-transactions-transaction-functions[transaction functions].
