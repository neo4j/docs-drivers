...
When a full certificate is installed, and encryption is enabled on the driver, full certificate checks are carried out (refer to link:/docs/operations-manual/4.0/security/ssl-framework[Operations Manual -> SSL framework]).
...
====
This example will assume that *Neo4j* has been configured for server policies as described in link:/docs/operations-manual/4.0/clustering-advanced/multi-data-center/load-balancing[Neo4j Operations Manual -> Load balancing for multi-data center systems].
...
====
A prerequisite for using a routing driver with routing context is that the *Neo4j* database is operated on a link:/docs/operations-manual/4.0/clustering[Causal Cluster] with the link:/docs/operations-manual/4.0/clustering-advanced/multi-data-center[Multi-data center licensing option] enabled.
Additionally, the routing contexts must be defined within the cluster as *_routing policies_*.

For details on how to configure *_multi-data center routing policies_* for a Causal Cluster, please refer to link:/docs/operations-manual/4.0/clustering-advanced/multi-data-center/load-balancing[Operations Manual -> Causal Clustering].
====
...
link:/docs/operations-manual/4.0/reference/configuration-settings#config_dbms.default_advertised_address[`default_advertised_address`]
