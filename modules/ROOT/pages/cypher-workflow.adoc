...
The Neo4j Drivers expose a Cypher Channel over which database work can be carried out (see the link:docs/cypher-manual/4.1/index.adoc[Cypher Manual] for more information
...
Sessions are *_lightweight containers for causally chained sequences of transactions_* (see link:docs/operations-manual/4.1/clustering/introduction/index.adoc#causal-consistency-explained[Operations Manual -> Causal consistency]).
...
The session classes are described in xref:session-api/index.adoc[The session API].
For more details, please see the xref:get-started.adoc#driver-api-docs[API documentation] for your language.
...
It is also the required form of transaction for running link:docs/cypher-manual/4.1/query-tuning/using/index.adoc#query-using-periodic-commit-hint[`PERIODIC COMMIT`] queries,
...
To learn more about how to use unmanaged transactions, see xref:get-started.adoc#driver-api-docs[API documentation] for the relevant language.
...
Parameter values should adhere to link:docs/cypher-manual/4.1/syntax/values/index.adoc[].
...
See link:docs/cypher-manual/4.1/query-tuning/how-do-i-profile-a-query/index.adoc[Cypher Manual -> Profiling a query] for more information on these queries.
...
For more information on Causal Clusters, please refer to link:docs/operations-manual/4.1/clustering/index.adoc[Operations Manual -> Clustering].
...
For more information about managing multiple databases within the same DBMS, refer to link:docs/cypher-manual/4.1/introduction/neo4j-databases-graphs/index.adoc[Cypher Manual -> Neo4j databases and graphs], which has a full breakdown of the *Neo4j* data storage hierarchy.
...
The database selection on the client side happens on the xref:session-api/index.adoc[session API].
...
The following example illustrates the concept of DBMS link:docs/cypher-manual/4.1/introduction/transactions/index.adoc[Cypher Manual -> Transactions] and shows how queries to multiple databases can be issued in one driver transaction. It is annotated with comments describing how each operation impacts database transactions.
...
For information on how to work with null in Cypher, please refer to link:docs/cypher-manual/4.1/syntax/working-with-null/index.adoc[Cypher Manual -> Working with `null`].
...
*_Server-generated exceptions_* are each associated with a link:docs/status-codes/4.1/index.adoc#status-codes[*_status code_*] that describes the nature of the problem and a message that provides more detail.
