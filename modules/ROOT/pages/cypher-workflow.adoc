...
The Neo4j Drivers expose a Cypher Channel over which database work can be carried out (see the link:/docs/cypher-manual/4.1[Cypher Manual] for more information
...
Sessions are *_lightweight containers for causally chained sequences of transactions_* (see link:/docs/operations-manual/4.1/clustering/introduction#causal-consistency-explained[Operations Manual -> Causal consistency]).
...
It is also the required form of transaction for running link:/docs/cypher-manual/4.1/query-tuning/using#query-using-periodic-commit-hint[`PERIODIC COMMIT`] queries,
...
Parameter values should adhere to link:/docs/cypher-manual/4.1/syntax/values[].
...
See link:/docs/cypher-manual/4.1/query-tuning/how-do-i-profile-a-query[Cypher Manual -> Profiling a query] for more information on these queries.
...
For more information on Causal Clusters, please refer to link:/docs/operations-manual/4.1/clustering[Operations Manual -> Clustering].
...
For more information about managing multiple databases within the same DBMS, refer to link:/docs/cypher-manual/4.1/introduction/neo4j-databases-graphs[Cypher Manual -> Neo4j databases and graphs], which has a full breakdown of the *Neo4j* data storage hierarchy.
...
The following example illustrates the concept of DBMS link:/docs/cypher-manual/4.1/introduction/transactions[Cypher Manual -> Transactions] and shows how queries to multiple databases can be issued in one driver transaction. It is annotated with comments describing how each operation impacts database transactions.
...
For information on how to work with null in Cypher, please refer to link:/docs/cypher-manual/4.1/syntax/working-with-null[Cypher Manual -> Working with `null`].
...
*_Server-generated exceptions_* are each associated with a link:/docs/status-codes/4.1#status-codes[*_status code_*] that describes the nature of the problem and a message that provides more detail.
