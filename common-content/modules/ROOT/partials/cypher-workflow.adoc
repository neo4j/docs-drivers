...
The Neo4j Drivers expose a Cypher Channel over which database work can be carried out (see the link:{neo4j-docs-base-uri}/cypher-manual/5.0[Cypher Manual] for more information
...
*_Server-generated exceptions_* are each associated with a link:{neo4j-docs-base-uri}/status-codes/5.0#status-codes[*_status code_*] that describes the nature of the problem and a message that provides more detail.
