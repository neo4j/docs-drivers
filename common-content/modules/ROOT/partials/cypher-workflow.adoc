...
The Neo4j Drivers expose a Cypher Channel over which database work can be carried out (see the link:docs/cypher-manual/5.0/index.adoc[Cypher Manual] for more information
...
This effect is known as xref:terminology/index.adoc#term-causal-chaining[*_causal chaining_*].
...
Sessions are *_lightweight containers for causally chained sequences of transactions_* (see link:docs/operations-manual/5.0/clustering/introduction/index.adoc#causal-consistency-explained[Operations Manual -> Causal consistency]).
...
In languages where xref:terminology/index.adoc#term-thread-safety[thread safety] is an issue, *_sessions should not be considered thread-safe_*.
...
The session classes are described in xref:session-api/configuration/index.adoc#driver-session-api[The session API].
...
This is the responsibility of the xref:terminology/index.adoc#term-transaction-manager[*_transaction manager_*].

The Neo4j Drivers provide *_transaction management_* via the xref:terminology/index.adoc#term-transaction-function[*_transaction function_*] mechanism.
...
A convenient *_short-form alternative_* is the xref:terminology/index.adoc#term-auto-commit[*_auto-commit_*] transaction mechanism.
...
It is also the required form of transaction for running link:docs/cypher-manual/5.0/clauses/call-subquery/index.adoc#subquery-call-in-transactions[Cypher Manual -> `CALL {} IN TRANSACTIONS`] queries, which are the only type of Cypher Query to manage their own transactions.
...
Parameter values should adhere to link:docs/cypher-manual/5.0/syntax/values/index.adoc[Cypher values].
...
See link:docs/cypher-manual/5.0/query-tuning/how-do-i-profile-a-query/index.adoc[Cypher Manual -> Profiling a query] for more information on these queries.
...
For more information on Causal Clusters, please refer to link:docs/operations-manual/5.0/clustering/index.adoc[Operations Manual -> Clustering].
...
Transactions can be executed in either *_read_* or *_write_* mode; this is known as the xref:terminology/index.adoc#term-access-mode[*_access mode_*].
...
For more information about managing multiple databases within the same DBMS, refer to link:docs/cypher-manual/5.0/introduction/neo4j-databases-graphs/index.adoc[Cypher Manual -> Neo4j databases and graphs], which has a full breakdown of the *Neo4j* data storage hierarchy.
...
The database selection on the client side happens on the xref:session-api/configuration/index.adoc#driver-session-api[session API].
...
The following example illustrates the concept of DBMS link:docs/cypher-manual/5.0/introduction/transactions/index.adoc#dbms-transactions[Cypher Manual -> Transactions] and shows how queries to multiple databases can be issued in one driver transaction. It is annotated with comments describing how each operation impacts database transactions.
...
For information on how to work with null in Cypher, please refer to link:docs/cypher-manual/5.0/syntax/working-with-null/index.adoc[Cypher Manual -> Working with `null`].
...
*_Server-generated exceptions_* are each associated with a link:docs/status-codes/5.0/index.adoc#status-codes[*_status code_*] that describes the nature of the problem and a message that provides more detail.
