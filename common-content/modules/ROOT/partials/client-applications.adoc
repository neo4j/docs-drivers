...
In languages where xref:terminology.adoc#term-thread-safety[thread safety] is an issue, the Driver Object can be considered *_thread-safe_*.
...
When a full certificate is installed, and encryption is enabled on the driver, full certificate checks are carried out (refer to link:/docs/operations-manual/4.4/security/ssl-framework[Operations Manual -> SSL framework]).
...
image::{common-image}/dns-resolution.png[title="Initial address resolution over DNS", role="middle"]
...
image::{common-image}/custom-middleware.png[title="Initial address resolution using custom middleware"]
...
image::{common-image}/resolver-function.png[title="Initial address resolution using resolver function"]
...
====
This example will assume that *Neo4j* has been configured for server policies as described in link:/docs/operations-manual/4.4/clustering-advanced/multi-data-center/load-balancing[Neo4j Operations Manual -> Load balancing for multi-data center systems].
...
====
A prerequisite for using a routing driver with routing context is that the *Neo4j* database is operated on a link:/docs/operations-manual/4.4/clustering[Causal Cluster] with the link:/docs/operations-manual/4.4/clustering-advanced/multi-data-center[Multi-data center licensing option] enabled.
Additionally, the routing contexts must be defined within the cluster as *_routing policies_*.

For details on how to configure *_multi-data center routing policies_* for a Causal Cluster, please refer to link:/docs/operations-manual/4.4/clustering-advanced/multi-data-center/load-balancing[Operations Manual -> Causal Clustering].
====
...
link:/docs/operations-manual/4.4/reference/configuration-settings#config_dbms.default_advertised_address[`default_advertised_address`]
...
The `ROUTING_CONTEXT` option allows for customization of the routing table and is discussed in more detail in xref:client-applications.adoc#driver-routing-context[Routing context].
...
[NOTE]
--
The Kerberos authentication token can only be understood by the server if the server has the link:{neo4j-docs-base-uri}/kerberos-add-on[Kerberos Add-on] installed.
--

# end::authentication-kerberos-note[]

[role=enterprise-edition]
[[driver-authentication-bearer]]
=== Bearer authentication

# tag::authentication-bearer[]

The bearer scheme allows you to create a bearer authentication token using a base64 encoded bearer authentication token provided by an Identity Provider.
This scheme is used in conjunction with Neo4j's Single Sign-On feature.

[NOTE]
--
The bearer scheme requires configuring Single Sign-On on the server.
See the Operations Manual for information on how to configure it link:/docs/operations-manual/{neo4j-version}/authentication-authorization/sso-integration[Operations Manual -> Integration with Single Sign-On services^].
Once configured, clients can discover Neo4j's configuration through the Discovery API.
--

The best way to create a bearer authentication token is shown below:

# end::authentication-bearer[]
...
Specify a custom server address resolver used by the routing driver to resolve the initial address used to create the driver. See xref:client-applications.adoc#driver-resolver-function[Resolver function] for more details.
