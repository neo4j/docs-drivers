...
GraphDatabase.driver("neo4j://graph.example.com:7687", auth)
----
|This is the default for Neo4j 5.x series
...
Possible values are:
+
* `neo4j.TrustSystemCAs()` - Accept any certificate that can be verified against the system store.
* `neo4j.TrustCustomCAs("/path/ca1.crt", "/path/ca2.crt", ...)` - Accept certificates at specified paths.
* `neo4j.TrustAll()` - Accept any certificate, including self-signed ones. *Not recommended for production environments.*


+
*Default:* `neo4j.TrustSystemCAs()` (Only when encryption is enabled)
