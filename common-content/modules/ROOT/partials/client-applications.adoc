...
In languages where xref:terminology/index.adoc#term-thread-safety[thread safety] is an issue, the Driver Object can be considered *_thread-safe_*.
...
To construct a driver instance, a xref:client-applications/index.adoc#driver-connection-uris[connection URI] and xref:client-applications/index.adoc#driver-authentication[authentication information] must be supplied.
...
When a full certificate is installed, and encryption is enabled on the driver, full certificate checks are carried out (refer to link:docs/operations-manual/5.0/security/ssl-framework/index.adoc[Operations Manual -> SSL framework]).
...
For more information, see xref:client-applications/index.adoc#driver-configuration-examples[Examples] below.
====
...
====
This example will assume that *Neo4j* has been configured for server policies as described in link:docs/operations-manual/5.0/clustering-advanced/multi-data-center/load-balancing/index.adoc[Neo4j Operations Manual -> Load balancing for multi-data center systems].
...
====
A prerequisite for using a routing driver with routing context is that the *Neo4j* database is operated on a link:docs/operations-manual/5.0/clustering/index.adoc[Causal Cluster] with the link:docs/operations-manual/5.0/clustering-advanced/multi-data-center/index.adoc[Multi-data center licensing option] enabled.
Additionally, the routing contexts must be defined within the cluster as *_routing policies_*.

For details on how to configure *_multi-data center routing policies_* for a Causal Cluster, please refer to link:docs/operations-manual/5.0/clustering-advanced/multi-data-center/load-balancing/index.adoc[Operations Manual -> Causal Clustering].
====
...
The `ROUTING_CONTEXT` option allows for customization of the routing table and is discussed in more detail in xref:client-applications/index.adoc#driver-routing-context[Routing context].
...
Specify a custom server address resolver used by the routing driver to resolve the initial address used to create the driver. See xref:client-applications/index.adoc#driver-resolver-function[Resolver function] for more details.
