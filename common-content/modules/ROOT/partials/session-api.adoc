...
[NOTE]
====
Unlike other kinds of Cypher query, `PERIODIC COMMIT` and `CALL {} IN TRANSACTIONS` queries do not participate in the causal chain.
Therefore, the only way to execute these queries from a driver is to use auto-commit transactions.

Please refer to the following for more details:

* link:docs/cypher-manual/4.4/query-tuning/using/index.adoc#query-using-periodic-commit-hint[Cypher Manual -> `PERIODIC COMMIT` query hint]
* link:docs/cypher-manual/4.4/clauses/call-subquery/index.adoc#subquery-call-in-transactions[Cypher Manual -> `CALL {} IN TRANSACTIONS`]
====
...
====
+
See link:docs/operations-manual/4.4/manage-databases/introduction/index.adoc#manage-databases-default[Operations Manual -> The default database] for more information on databases.
+
See below and link:docs/cypher-manual/4.4/access-control/dbms-administration.adoc#access-control-dbms-administration-impersonation[Cypher Manual -> The `DBMS IMPERSONATE` privileges] for more information on impersonation.
+
...
+
See link:docs/operations-manual/4.4/manage-databases/introduction/index.adoc#manage-databases-home[Operations Manual -> Per-user home databases] for more information on default database per-user.
+
...
(see link:docs/operations-manual/4.4/monitoring/transaction-management/index.adoc[Operations Manual -> Transaction Management]).
