...
Please refer to the <<cypher-manual#query-using-periodic-commit-hint, Cypher Manual -> `PERIODIC COMMIT` query hint>> and
<<cypher-manual#subquery-call-in-transactions, Cypher Manual -> `CALL {} IN TRANSACTIONS`>> for more details.
====
