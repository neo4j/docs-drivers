...
The bearer scheme allows you to create a bearer authentication token
using a base64 encoded bearer authentication token provided by an Identity Provider.
...
Please see the operations manual on how to configure it: <<https://development.neo4j.dev/docs/operations-manual/current/authentication-authorization/sso-integration/>>. Once configured, clients can discover Neo4j's configuration through the Discovery API.
--
