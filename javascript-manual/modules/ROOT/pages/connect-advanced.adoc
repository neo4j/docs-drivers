...
To work with rotating tokens, you need to provide a link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/auth-token-manager.js~AuthTokenManager.html[`AuthTokenManager`] instance when instantiating the `Driver`, rather than a static authentication token.
...
[NOTE]
This API must not be used for switching users.
...
For more information on what information is transmitted, see link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/types.js~Config.html#instance-member-telemetryDisabled[API documentation -> `telemetryDisabled`].
