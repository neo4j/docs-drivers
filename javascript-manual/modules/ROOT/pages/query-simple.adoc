...
An exception is raised if the operation keeps failing after a number of attempts.

All exceptions coming from the server are subclasses of link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/error.js~Neo4jError.html[`Neo4jError`].
...
let err = await driver.executeQuery(
  ...
  console.log('Exception GQL cause:', err.cause.message)
