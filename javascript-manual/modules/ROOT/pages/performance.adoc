...
// Bad practice
for(let i=0; i<10000; i++) {
  session.run('MERGE (:Number {value: $value})', { value: Math.random() })
}
----
+
[TIP]
...
session.run('CREATE INDEX personName FOR (n:Person) ON (n.name)')
----
...
----
const result = session.run('PROFILE MATCH (p {name: $name}) RETURN p', { name: 'Alice' })
...
[source, javascript, role=nocollapse]
----
const result = session.run('EXPLAIN MATCH (p {name: $name}) RETURN p', { name: 'Alice' })
