...
Including the driver in the page makes a global `neo4j` object available, through which you can then xref:connect.adoc[connect] and xref:query-simple.adoc[query] a Neo4j database in the same way as you would do for a local application.
...
The browser version is also exported as an ECMA Script Module.
...
You don't need to explicitly close the driver on a web page, unless its lifetime does not correspond to the lifetime of the page.
