...
[TIP]
For driver version >= 5.24 and server version >= 5.23, two forms of notifications are available (_Neo4j status codes_ and _GQL status codes_).
...
With version >= 5.24, the property `ResultSummary.gqlStatusObjects` contains a list of link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/notification.js~GqlStatusObject.html[`GqlStatusObject`]s.
