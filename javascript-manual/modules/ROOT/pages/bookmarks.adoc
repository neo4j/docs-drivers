...
To ensure causal consistency among transactions executed partly with `.executeQuery()` and partly with sessions, create the session with `bookmarkManager=driver.executeQueryBookmarkManager`.
