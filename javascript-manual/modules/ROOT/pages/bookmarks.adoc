...
async function createFriends(driver) {
  ...
  const sessionA = driver.session({database: '<database-name>'})
  try {
  ...
  const sessionB = driver.session({database: '<database-name>'})
  try {
...
To ensure causal consistency among transactions executed partly with `.executeQuery()` and partly with sessions, create the session with the option `bookmarkManager` and set it to `driver.executeQueryBookmarkManager`.
