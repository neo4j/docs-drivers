...
The type link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/vector.js~Vector.html[`Vector`] maps to the Cypher type link:https://neo4j.com/docs/cypher-manual/25/values-and-types/vector/[`VECTOR`].
`Vector` objects are stored as contiguous blocks of memory containing homogeneous values.

The `vector` constructor takes a Javascript link:https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray[`TypedArray`].
Supported `TypedArray` objects are: `Int8Array`, `Int16Array`, `Int32Array`, `Int64Array`, `Float32Array`, `Float64Array`.
...
var neo4jVector = neo4j.vector(typedArray)  // <.>
...
[NOTE]
Whereas regular Javascript arrays are capped when stringified, the `Vector` type always shows all elements.
Logging `Vector` objects risks clogging the log or resulting in Out of Memory errors.
...
The Cypher type link:https://neo4j.com/docs/cypher-manual/current/values-and-types/uuid/[`UUID`] maps to link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/uuid.js~UUID.html[`UUID`].

.Store and work with a UUID
[source, javascript]
----
// Store UUID as a node property and get it back
var { records } = await driver.executeQuery(
    'MERGE (d:Doc {id: $uuid}) RETURN d.id AS uuid',
    { uuid: neo4j.uuid('12345678-1234-1234-1234-123456789abc'), }
)
...
The type link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/unsupported-type.js~UnsupportedType.html[`UnsupportedType`] is used for data types returned by Cypher queries, but that the driver doesn't recognize.
