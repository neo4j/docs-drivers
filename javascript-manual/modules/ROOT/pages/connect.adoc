...
(async () => {
  ...
  console.log(await driver.verifyConnectivity())  # <2>
...
Both the creation of a `Driver` object and the connection verification can raise exceptions.
Since a connection error is a blocker for any subsequnt task, the most common choice is to let the program crash should an exception occurr during connection.
