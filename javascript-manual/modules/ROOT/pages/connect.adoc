...
**`Driver` objects are immutable, thread-safe, and expensive to create,** , so your application should create only one instance.
...
To connect to such an instance, you can either manually extract the URI and the credentials from that file, or use a third party-module to load them as environment variables.
We recommend the package link:https://www.npmjs.com/package/dotenv/[`dotenv`] for that purpose.
