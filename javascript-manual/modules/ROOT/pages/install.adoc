...
[IMPORTANT]
Import `Node` explicitly to avoid conflicts with the native `Node` type. The driver also doesn't declare a namespace, so you need to explicitly import any symbol you want to use as a type annotation.
