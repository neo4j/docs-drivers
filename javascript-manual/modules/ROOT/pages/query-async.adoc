...
try {
  peopleRecords = session.executeWrite(async tx => {
...
can't read results twice, it's a cursor
processing happens inside transaction function
...
let session = driver.rxSession()  // <1>
let rxResult = session.executeRead(tx => {
  return tx
