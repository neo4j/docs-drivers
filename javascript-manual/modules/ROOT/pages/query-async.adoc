...
----
const session = driver.session({database: 'neo4j'})
const records = session.executeWrite(tx => {
  let peopleRecords = []
  tx
...
Notice that *the processing of the result happens _inside_ the <<transaction_function>>*. You *should not* return the raw result out of the transaction function and _then_ subscribe to it.
