...
* xref:session-api.adoc#driver-simple-sessions[Simple sessions]
** xref:session-api.adoc#driver-simple-lifecycle[Lifecycle]
** xref:session-api.adoc#driver-simple-transaction-fn[Transaction functions]
** xref:session-api.adoc#driver-simple-autocommit-transactions[Auto-commit transactions]
** xref:session-api.adoc#driver-simple-result-consume[Consuming results]
** xref:session-api.adoc#driver-simple-result-retain[Retaining results]
* xref:session-api.adoc#driver-async-sessions[Asynchronous sessions]
** xref:session-api.adoc#driver-async-transaction-fn[Transaction functions]
** xref:session-api.adoc#driver-async-autocommit-transactions[Auto-commit transaction]
** xref:session-api.adoc#driver-async-result-consume[Consuming results]
* xref:session-api.adoc#driver-rx-sessions[Reactive sessions]
** xref:session-api.adoc#driver-rx-lifecycle[Lifecycle]
** xref:session-api.adoc#driver-rx-transaction-fn[Transaction functions]
** xref:session-api.adoc#driver-rx-autocommit-transactions[Auto-commit transactions]
** xref:session-api.adoc#driver-rx-result-consume[Consuming results]
** xref:session-api.adoc#driver-rx-cancellation[Cancellation]
* xref:session-api.adoc#driver-session-configuration[Configuration]
...
*_When a transaction fails, the driver retry logic is invoked_*. For several failure cases, the transaction can be immediately retried against a different server.
...
Please refer to the link:docs/cypher-manual/4.0/query-tuning/using/index.adoc#query-using-periodic-commit-hint[Cypher Manual -> `PERIODIC COMMIT` query hint].
====
...
See link:docs/operations-manual/4.0/manage-databases/introduction/index.adoc#manage-databases-default[Operations Manual -> The default database] for more information on databases.
+
