...
To hold on to the results while running additional statements, or to use the results outside the scope of the current transaction, the results must be retained by assigning them to a variable.


[[driver-result-cursor]]
== The result cursor

A result cursor provides access to the stream of records that comprise the results.
The cursor is advanced through the stream, pointing to each record in turn.
Before the cursor is moved onto the first record, it points _before_ the first record.
Summary information and metadata are available once the cursor has traversed to the end of the result stream.

[NOTE]
--
Result records are loaded lazily as the cursor is advanced through the stream.
This means that the cursor must be moved to the first result before this result can be consumed.
It also means that the entire stream must be explicitly consumed before summary information and metadata are available.
...
The result record stream is comprised of individual records.
...
Metadata can be retrieved along with the result stream, an example being the query plan obtained by prepending a Cypher query with `PROFILE` or `EXPLAIN`.
...
[.tabbed-example]
.Profile query and print execution plan
====
[.include-with-dotnet]
======
[source, csharp]
...
The driver additionally exposes timings for query execution, such as `resultAvailableAfter` which returns the time taken by the server to make the result available for consumption.
Further details may be found in the driver API documentation.