...
For each statement results may be processed and any exceptions are handled.
...
A Session is a wrapper around a connection that can be used to group units of work by a common end point.
...
Deploying against a Causal Cluster this means that the driver can route reads and writes efficiently.
