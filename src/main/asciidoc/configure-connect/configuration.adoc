...
= Configuration
:configuring-bolt-connectors-uri: {operations-manual-base-uri}/configuration/connectors/

Configuration settings are supplied to the driver constructor.
We illustrate the use of these in the examples that follow in the remainder of this section.

[[driver-authentication-encryption]]
== Encryption

All Neo4j drivers can be configured to turn on or off TLS encryption.
...
This makes it clear encryption is used when other developers read the code, and minimizes the risk of mistakes arising by relying on defaults.

[.tabbed-example]
.Configure the driver to require TLS encryption
====
[.include-with-dotnet]
======
[source, csharp]
...
Without verifying this, an attacker can easily perform a so-called "man-in-the-middle" attack, tricking the driver into establishing an encrypted connection with her instead of the Neo4j instance.
...
[.tabbed-example]
.Configure the driver to trust on first use
====
[.include-with-dotnet]
======
[source, csharp]
...
[.tabbed-example]
.Configure the driver to trust signed certificate
====
[.include-with-dotnet]
======
[source, csharp]
...
[.tabbed-example]
.Connecting the driver to a server with authentication disabled
====
[.include-with-dotnet]
======
[source, csharp]
...
Given a fresh Neo4j server installation, any attempt to connect to the Neo4j server with the default password will provide a notification indicating that the credentials have expired and that the password needs to be updated.
