...
There are two kinds of Bolt URIs: direct URIs and routing URIs.
A direct URI is used to connect to a single instance.
If the application relies on routing and load balancing, these must be provided separately.
The driver is not aware of any cluster it might be connected to, only of the direct connection to a single server.
...
After providing the driver with a seed URI, it connects and requests an overview of the cluster topology.
