...
= Configuration
:configuring-bolt-connectors-uri: {operations-manual-base-uri}/deployment/#bolt-connector

// TODO: Confirm which configuration items are actually _uniform_.

In addition to the Bolt URI, the driver can be configured for:
...
| Session pool size (*) | Max number of sessions per URI.            | 50
