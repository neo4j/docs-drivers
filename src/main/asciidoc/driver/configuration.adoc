...
= Configuration
:configuring-bolt-connectors-uri: {operations-manual-base-uri}/deployment/post-installation/#bolt-connector
...
There is no limit as to how many sessions can be created, but a maximum value limits how many sessions will be buffered after they are returned to the session pool.

[.tabbed-example]
.Create the driver with a configuration
====
[.include-with-dotnet]
======
[source, csharp]
