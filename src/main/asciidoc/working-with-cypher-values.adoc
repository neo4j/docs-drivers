...
| ByteArray | byte[]
| Date | CypherDate
| Time | CypherTimeWithOffset
| LocalTime | CypherTime
| DateTime* | CypherDateTimeWithOffset / CypherDateTimeWithZoneId**
| LocalDateTime | CypherDateTime
...
+**+ The variant used depends on whether the value contains a numeric offset from UTC or a time zone name string.
======
...
| LocalTime | datetime.time†† | datetime.time††
| DateTime | datetime.datetime† | datetime.datetime†
| LocalDateTime | datetime.datetime†† | datetime.datetime††
...
+***+ A `timespan` passed as a parameter will always be implicitly converted to `duration`.
