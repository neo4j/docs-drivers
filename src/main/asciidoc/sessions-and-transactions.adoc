...
[source, python]
----
include::{python-examples}/test_examples.py[tag=session]
----
======
====
...
Of these, only <<transaction_functions, transaction function>> can be automatically replayed on failure.


=== Auto-commit Transactions

An auto-commit transaction is a simple but limited form of transaction.
Such a transaction consists of only one Cypher statement, cannot be automatically replayed on failure, and cannot take part in a <<causal-chaining, causal chain>>.
...
[source, python]
----
include::{python-examples}/test_examples.py[tag=autocommit-transaction]
----
======
====
...
However, Auto-commit transactions are the only way to execute `<<query-periodic-commit, USING PERIODIC COMMIT>>` Cypher statements.
...
[source, python]
----
include::{python-examples}/test_examples.py[tag=transaction-function]
----
======
====
...
This retry capability can be <<max-retry-time, configured>> on Driver construction.
...
// !!!!!!!!!!!!!!!!!! Causal Cluster, Causal cluster or causal cluster?
...
Causal chaining is carried out internally by passing a <<term-bookmark, bookmark>> between transactions.
The bookmark records a point in transactional history.
When beginning a transaction on a server relative to a bookmark, transaction execution will block until the server has caught up with the bookmark stored in the session.
If the transaction is carried out successfully, the bookmark will be updated.
Should the transaction fail, no such update will be made.

It is possible both to construct a session based on an existing bookmark, and to extract a bookmark from a session.
// !!!!!!!!!!!!!!!!!! How do you pass bookmarks, or how can I find out how to? Example!
These functions allow multiple sessions to be chained together.
...
[source, python]
----
include::{python-examples}/test_examples.py[tag=read-write-transaction]
----
======
====
