...
NOTE: The async API is only available with the .NET and Java drivers.
...
It is therefore suggested to always do `Session#closeAsync()` at the end of the `CompletionStage` chain.
...
[.include-with-javascript]
======
This functionality is not available with this driver.
======
