...
[NOTE]
.Server-side configuration to enable load balancing
====
A prerequisite for Routing drivers is that the Neo4j database is operated on a <<operations-manual#causal-clustering, Causal Cluster>> with the <<operations-manual#multi-dc-intro, Multi-data center licensing option>> enabled.
Additionally, if using Routing drivers with routing context, the routing contexts must be first defined within the cluster.
For details on configuring a Causal Cluster to provide for load balancing, please refer to <<operations-manual#multi-dc-load-balancing, Operations Manual -> Causal Clustering>>.
====
